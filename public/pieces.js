(function(global){
  ...
  let serial=0;
  function letters(piece,game){
    const white=piece[0]==='w',type=piece[1],checker=game.startsWith('checkers');
    ...
    return `<svg class="artisan-piece artisan-thai-letters" viewBox="0 0 64 64" aria-hidden="true"><defs><linearGradient id="${id}" x1="0" y1="0" x2="1" y2=".8"><stop stop-color="${top}"/><stop offset=".28" stop-color="${mid}"/><stop offset=".55" stop-color="${top}"/><stop offset="1" stop-color="${bottom}"/></linearGradient></defs><ellipse cx="32" cy="56" rx="22" ry="4" fill="#071019" opacity=".26"/><circle cx="32" cy="33" r="23" fill="url(#${id})" stroke="${edge}" stroke-width="1.5"/><circle cx="32" cy="33" r="19" fill="none" stroke="${edge}" stroke-opacity=".45"/><text x="32" y="38" text-anchor="middle" font-family="Noto Sans Thai,sans-serif" font-weight="700" font-size="${label.length>4?13:16}" fill="${white?'#483820':'#f4e6ce'}">${label}</text></svg>`;
  }
  function renderPiece(piece,gameType,pieceSet='studio'){
    if(!piece)return '';
    if(pieceSet==='thai-letters')return letters(piece,gameType);
