(function (global) {
  ...
  function applyMove(board, fromR, fromC, toR, toC) {
    ...
    while (cr !== toR || cc !== toC) {
      if (newBoard[cr][cc]) { newBoard[cr][cc] = null; captured = true; }
      cr += dr; cc += dc;
    }
    let promoted = false;
  ...
  function gameStatus(board, currentPlayer) {
    if (countPieces(board, currentPlayer) === 0) return { ended: true, winner: currentPlayer === 'w' ? 'b' : 'w', reason: 'no_pieces' };
