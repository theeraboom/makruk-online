...
async function main(){await fs.mkdir(out,{recursive:true});for(const[name,engine,options]of[['chrome',chromium,{viewport:{width:1360,height:1050}}],['webkit-mobile',webkit,{...devices['iPhone 13']}]]){
  ...
  try{for(const game of ['chess','chess-intl','checkers','checkers-intl','connect4']){
    ...
    try{
      ...
      if(game==='connect4'){await clickSquare(page,f,1,1,false,name!=='chrome');await f.waitForFunction(()=>moves.length>0);assert.equal(await f.evaluate(()=>moves[0].to.c),1);}
      else {
      ...
      // Browsers can restore a page from the back/forward cache without
      // re-running room.js. Recreate the disposed viewer exactly once.
      const countBeforeRestore=await f.evaluate(()=>moves.length);
      await f.evaluate(()=>window.dispatchEvent(new PageTransitionEvent('pagehide',{persisted:true})));
      assert.equal(await f.locator('canvas.board-canvas').count(),0);
      await f.evaluate(()=>window.dispatchEvent(new PageTransitionEvent('pageshow',{persisted:true})));
      await f.waitForFunction(()=>boardScene&&document.getElementById('boardStage').classList.contains('has-3d'));
